...
const Dashboard = () => {
  ...
  const fetchAllSessions = async () => {
    try {
      ...
      // The response has the structure: { success: true, count: number, sessions: [...] }
      if (response.data && response.data.success && Array.isArray(response.data.sessions)) {
        setSessions(response.data.sessions);
      } else {
        console.error("Unexpected response format:", response.data);
        setSessions([]); // Set to empty array to prevent .map() errors
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        setSessions(prevSessions => 
          prevSessions.filter(session => session._id !== sessionData?._id)
        );
      } else {
